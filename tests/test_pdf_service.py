...
from app.pdf_service import PdfProcessingError, merge_pdf_files, split_pdf_by_page, split_pdf_by_range, zip_files
...
def test_split_pdf_by_range_writes_balanced_parts(tmp_path):
    source = write_pdf(tmp_path / "source.pdf", 5)
    output_dir = tmp_path / "parts"
    output_dir.mkdir()

    parts = split_pdf_by_range(source, output_dir, start_page=2, end_page=5, parts=3)

    assert [part.name for part in parts] == [
        "part_001_pages_002-003.pdf",
        "part_002_pages_004-004.pdf",
        "part_003_pages_005-005.pdf",
    ]
    assert [len(PdfReader(str(part)).pages) for part in parts] == [2, 1, 1]
